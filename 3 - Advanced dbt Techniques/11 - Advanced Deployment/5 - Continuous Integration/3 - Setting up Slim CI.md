...
2. Fill out the configurations necessary to set up a deployment type environment.
    - Job name - Slim CI Job
    - Environment - CI
    - Defer to a previous run state? - Choose a standard production environment run. (e.g. `Production(Production)`)
    - In commands, execute `dbt --select state:modified+`
